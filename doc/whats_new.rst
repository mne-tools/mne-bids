...
- Add ``datatype`` to replace ``kind`` parameter to ``mne_bids.make_bids_folders`` and :func:`mne_bids.read_raw_bids` that allows users to specify EEG, MEG, or iEEG datasets by `Adam Li`_ (`#514 <https://github.com/mne-tools/mne-bids/pull/514>`_)
