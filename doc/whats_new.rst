...
- Add :func:`mne_bids.read_epochs_bids` to read pre-epoched EEGLAB ``.set`` datasets (``"RecordingType": "epoched"``) into :class:`mne.Epochs`. :func:`mne_bids.read_raw_bids` now raises a clear error pointing to the new function on such datasets, by `Bruno Aristimunha`_ (:gh:`1605`)
