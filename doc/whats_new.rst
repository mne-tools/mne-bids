...
- The :func:`mne_bids.make_bids_basename` function has been updated to create a :code:`mne_bids.utils.BIDSPath` object, which operates like a path object storing the `basename` and `bids_root` of the path, adds `kind` and `extension` kwargs, can infer the full file path of a dataset, and allows dynamic updating of BIDs entities via the `update` function, by `Adam Li`_ (`#446 <https://github.com/mne-tools/mne-bids/pull/446>`_, `#492 <https://github.com/mne-tools/mne-bids/pull/492>`_)
