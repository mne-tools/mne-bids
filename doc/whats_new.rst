...
- :func:`mne_bids.read_raw_bids` is more tolerant of imperfect ``channels.tsv`` files: it warns and skips channel metadata when the file is empty, missing the ``name`` column, or has channel names that don't match the raw file (the latter via the new default ``on_ch_mismatch="warn"``); duplicate channel names are deduplicated with ``-0/-1/...`` suffixes. Pass ``on_ch_mismatch="raise"`` to restore strict behavior, by `Bruno Aristimunha`_ (:gh:`1603`)
