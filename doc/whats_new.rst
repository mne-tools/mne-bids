...
- :func:`mne_bids.read_raw_bids` is more tolerant of imperfect ``channels.tsv`` files: it warns and skips channel metadata when the file is empty or missing the ``name`` column, deduplicates repeated channel names with ``-0/-1/...`` suffixes, and accepts a new ``on_ch_mismatch="warn"`` option, by `Bruno Aristimunha`_ (:gh:`1603`)
