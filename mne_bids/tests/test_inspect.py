...
def setup_bids_test_dir(bids_root):
    ...
    write_raw_bids(
        raw, bids_path=bids_path, events=events, event_id=event_id, overwrite=True
    )
...
def test_inspect_auto_flats(tmp_path, save_changes):
    ...
    bids_root = setup_bids_test_dir(tmp_path)
    bids_path = _bids_path.copy().update(root=bids_root)
    channels_tsv_fname = bids_path.copy().update(suffix="channels", extension=".tsv")
    ...
    if save_changes:
        ...
        assert (
            channels_tsv_data["status_description"][10]
