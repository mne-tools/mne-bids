...
def test_update_sidecar_jsons(
    ...
    with pytest.raises(RuntimeError, match='Only works for ".json"'):
        update_sidecar_json(
            sidecar_path.copy().update(extension=None), _get_sidecar_json_update_file
        )
...
def test_update_anat_landmarks(tmp_path):
    ...
    bids_path_mri = write_anat(
        image=t1_path, bids_path=bids_path_mri, landmarks=landmarks, deface=False
    )
    ...
    assert np.allclose(
        landmarks_new.dig[1]["r"], mri_json["AnatomicalLandmarkCoordinates"]["NAS"]
    )
    ...
    with pytest.raises(
        KeyError, match="No AnatomicalLandmarkCoordinates section found"
    ):
        update_anat_landmarks(bids_path=bids_path_mri, landmarks=landmarks_new)
    ...
    update_anat_landmarks(bids_path=bids_path_mri_no_ext, landmarks=landmarks_new)
    ...
    with pytest.raises(ValueError, match='Can only operate on "T1w" and "FLASH"'):
        update_anat_landmarks(bids_path=bids_path_invalid, landmarks=landmarks)
    ...
    with pytest.raises(ValueError, match="must be specified in MRI voxel"):
        update_anat_landmarks(bids_path=bids_path_mri, landmarks=landmarks_invalid)

    # Missing cardinal point
    landmarks_invalid = landmarks.copy()
    del landmarks_invalid.dig[0]
    with pytest.raises(
        ValueError, match="did not contain all required cardinal points"
    ):
        update_anat_landmarks(bids_path=bids_path_mri, landmarks=landmarks_invalid)

    # Test with path-like landmarks
    fiducials_path = data_path / "subjects" / "sample" / "bem" / "sample-fiducials.fif"
    ...
    for landmark, expected_coords in zip(
        ("LPA", "NAS", "RPA"), expected_coords_in_voxels
    ):
        assert np.allclose(
