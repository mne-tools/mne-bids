...
from mne_bids.config import ENCODING_IN
...
def update_sidecar_json(bids_path, entries, verbose=None):
    ...
    if isinstance(entries, dict):
        ...
        with _open_lock(entries, encoding=ENCODING_IN) as tmp_f:
    ...
    with _open_lock(fpath, encoding=ENCODING_IN) as tmp_f:
...
def _get_landmarks_from_fiducials_file(
