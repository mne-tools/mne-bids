...
def run():
    ...
    parser.add_option("--raw", dest="raw_fname", help="path to the raw MEG file")
    parser.add_option(
        "--bids_root", dest="bids_root", help="The path of the BIDS compatible folder."
    )
    parser.add_option(
        "--session_id", dest="session_id", help="session name in BIDS compatible format"
    )
    ...
    parser.add_option("--hpi", dest="hpi", help="path to the MEG marker points")
    parser.add_option(
        "--electrode", dest="electrode", help="path to head-native digitizer points"
    )
    parser.add_option("--hsp", dest="hsp", help="path to headshape points")
    parser.add_option("--config", dest="config", help="path to the configuration file")
    parser.add_option(
        "--overwrite",
        dest="overwrite",
    ...
    if not all([opt.subject_id, opt.task, opt.raw_fname, opt.bids_root]):
        parser.print_help()
        parser.error(
            "Arguments missing. You need to specify at least the"
            "following: --subject_id, --task, --raw, --bids_root."
        )

    bids_path = BIDSPath(
        subject=opt.subject_id,
    ...
    if opt.overwrite is not None:
        truthy = [1, "True", "true"]
        falsy = [0, "False", "false"]
        bool_mapping = dict(chain(zip(truthy, repeat(True)), zip(falsy, repeat(False))))
